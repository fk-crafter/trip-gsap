...
export default function OceanSection() {
  ...
  return (
    <section
      ref={containerRef}
      className="relative w-full h-screen overflow-hidden bg-gradient-to-b from-[#d4f1f9] to-[#fff5e1] text-gray-800 z-50"
    >
      <div className="absolute top-16 left-1/2 -translate-x-1/2 text-center z-10">
        <h2 className="text-4xl font-bold tracking-tight text-sky-700">
          Ocean Vibes
        </h2>
        <p className="text-sky-600 mt-2 max-w-md mx-auto text-sm">
        ...
        {destinations.map((dest, index) => (
          ...
          >
            ...
            <div className="absolute bottom-4 left-1/2 -translate-x-1/2 z-20">
              <p className="text-white text-lg font-semibold bg-sky-700/60 px-4 py-1 rounded-full backdrop-blur-sm shadow-md">
                {dest.title}
              </p>
            </div>
          </div>
